/**
 * Module quản lý Chính sách PT (sheet: "chinh_sach_PT")
 * Cột: Id, hang_thanh_vien, diem_tich_luy, chiet_khau_khm, chiet_khau_doanh_thu, quyen_loi, uu_dai, qua_tang,
 * thoi_gian_tao, ngay_tao, nguoi_tao, thang, nam, tinh_trang, IsDeleted
 */

function getPolicyPTs() {
  try {
    var ss = SpreadsheetApp.openById(SPREADSHEET_ID);
    var sheet = ss.getSheetByName('chinh_sach_PT');
    if (!sheet) throw new Error("Sheet 'chinh_sach_PT' không tồn tại.");
    ...
    var list = rows.map(function(row, idx) {
      ...
      return {
        ...
        chiet_khau_doanh_thu: row[headerMap['chiet_khau_doanh_thu']],
...
function addPolicyPT(data) {
  try {
    ...
    setValue('chiet_khau_doanh_thu', data.chiet_khau_doanh_thu);
    ...
    return { success: true, message: 'Thêm chính sách PT thành công!' };
  } catch (e) {
    return { success: false, message: e.message };
  }
}

function updatePolicyPT(data) {
  try {
    var ss = SpreadsheetApp.openById(SPREADSHEET_ID);
    var sheet = ss.getSheetByName('chinh_sach_PT');
    if (!sheet) throw new Error("Sheet 'chinh_sach_PT' không tồn tại.");
    ...
    if (headerMap['chiet_khau_doanh_thu'] !== undefined) values[headerMap['chiet_khau_doanh_thu']] = data.chiet_khau_doanh_thu || '';
    ...
    return { success: true, message: 'Cập nhật chính sách PT thành công!' };
  } catch (e) {
    return { success: false, message: e.message };
  }
}

function deletePolicyPT(rowNumber) {
  try {
    var ss = SpreadsheetApp.openById(SPREADSHEET_ID);
    var sheet = ss.getSheetByName('chinh_sach_PT');
    if (!sheet) throw new Error("Sheet 'chinh_sach_PT' không tồn tại.");
    ...
    return { success: true, message: 'Xóa chính sách PT thành công!' };
  } catch (e) {
    return { success: false, message: e.message };
  }
}
